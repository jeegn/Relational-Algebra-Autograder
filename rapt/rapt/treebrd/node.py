...
class RelationNode(Node):
    ...
    def __init__(self, name, schema, alias = ''):
        super().__init__(Operator.relation, name)
        self.alias = alias
        self.attributes = AttributeList(schema.get_attributes(name), alias or name)
...
class UnaryNode(Node):
    ...
    def __init__(self, operator, child, name=None):
        ...
        if not name:
            ...
            if hasattr(self.child, 'alias'):
                self.alias = self.child.alias
...
class ThetaJoinNode(JoinNode):
    ...
    def __init__(self, left, right, conditions):
        ...
        self.attributes.validate(get_attribute_references(conditions))
        self.conditions = conditions
        # If theta join in based on a equality condition, it is equivalent to a natural join
        # So, remove the duplicate attributes
        # if not any(op in self.conditions for op in ('<', '>', '!')):
        #     left_attributes = [attribute.prefixed
        #                        for attribute in self.left.attributes]
        #     left_names = self.left.attributes.names
        #     right_attributes = [attribute.prefixed
...
class ThetaRightOuterJoinNode(JoinNode):
    """
    A relation that results from the relation algebra theta right outer join operator.
    """

    def __init__(self, left, right, conditions):
        super().__init__(Operator.theta_right_outer_join, left, right)
        self.attributes.validate(get_attribute_references(conditions))
        self.conditions = conditions
...
class ThetaLeftOuterJoinNode(JoinNode):
    """
    A relation that results from the relation algebra theta left outer join operator.
    """

    def __init__(self, left, right, conditions):
        super().__init__(Operator.theta_left_outer_join, left, right)
        self.attributes.validate(get_attribute_references(conditions))
        self.conditions = conditions
...
class ThetaFullOuterJoinNode(JoinNode):
    """
    A relation that results from the relation algebra theta full outer join operator.
    """

    def __init__(self, left, right, conditions):
        super().__init__(Operator.theta_full_outer_join, left, right)
        self.attributes.validate(get_attribute_references(conditions))
        self.conditions = conditions
...
class SetOperatorNode(BinaryNode):
    ...
    def __init__(self, operator, left, right):
        ...
        if not len(left.attributes) == len(right.attributes):
            raise InputError(
                f'Set operations require an equal number of attributes across the relations.\nThe schema of the left relation is: {left.attributes.to_list()}\nThe schema of the right relation is: {right.attributes.to_list()}')
...
class Operator(Enum):
    ...
    right_outer_join = 28
    full_outer_join = 29
    theta_right_outer_join = 30
    theta_left_outer_join = 31
    theta_full_outer_join = 32
